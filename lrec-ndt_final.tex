...
in collaboration with the University of Oslo. It is the first available treebank for Norwegian.
This paper presents the core principles behind the syntactic annotation and how these principles were employed in certain specific cases. We then go on to the selection of texts and distribution between genres, and
...
verbs, discussed above, is a case where the lexical word is chosen as
...
constituents and in the case of coordination is more or less the same
...
A prominent case where a function word is head, is sentences with a
...
exemplifies the annotation choices mentioned in this subsection. We
find the coordinate structure {\it et eple og en pære} 'an apple and a
pear', where the first conjunct heads the coordination, nominal
phrases, such as {\it et eple} 'an apple', where the noun heads the
structure, as well as the finite auxiliary {\it har} 'has', which
...
development. We also find that the \newcite{Boh:Niv:12} parser obtains
