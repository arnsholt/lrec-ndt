...
Norwegian.\footnote{A treebank of deep linguistic analysis couched in
  ...
  by the INNESS project.} Hence, the progress in parsing and applications
described above has not been possible for Norwegian.  At present,
however, Spr{\aa}kbanken, at the Norwegian National Library,
...
Dependency Treebank (NDT), a syntactic treebank which encompasses
treebanks for both variants of Norwegian (Bokm{\aa}l and
Nynorsk).\footnote{These are the two written varieties of Norwegian.}\footnote{In the development phase, the treebank has also been referred to as Språkbanken's Gold Standard Corpus.}
...
In the NDT, sentences such as (\ref{ex:medat}) and (\ref{ex:utenat}) are analyzed similarly: The (finite) verb of the clausal complement serves as head in both cases, and carries the dependency relation DOBJ (direct object), c.f. figure \ref{figure:medat} and \ref{figure:utenat}.
% Both will therefore be retrieved through a query for finite verbs with the dependency relation DOBJ.
Table (x) summarizes some choices where dependency treebanks tend to differ.
...
\begin{table}[!h]
  ...
  \begin{tabular}{|l|l|l|}
    ...
    \hline
    Transitive preposition & Preposition & Prepositional complement \\
    \hline
    Subordinate clauses & Finite verb & Complementizer \\
    \hline
    Coordination & First conjunct & Subsequent conjuncts \\
    \hline
    Determiner-noun complex & Noun & Determiner \\
    \hline
  \end{tabular}
...
\ctable[botcap,
    ...
    mincapwidth=\columnwidth
        ...
        \FL
        Source & \multicolumn{2}{c}{Fraction} \ML
